...
Given("A plant named {string} exists in the list", (plantName) => {
  return ensurePlantExistsInList(plantName).then(() => {
    cy.get("table tbody tr")
      .contains("td", plantName, { timeout: 10000 })
      .should("be.visible")
      .then(($cell) => {
...
When(
  ...
  (text) => {
    ...
    plantPage.searchInput.clear().type(value);
  },
);
...
When('Click the "Edit" button on a plant row', () => {
  if (selectedPlantRow) {
    ...
    plantPage.clickEditForPlant("Banana");
  }
...
When('Update the "Category" field to {string}', (category) => {
  const desiredLower = plantPage.normalizeSpaces(category).toLowerCase();
  ...
  addPlantPage
    ...
    .then(($select) => {
      ...
      if (!value) {
        throw new Error(
          `Category '${category}' was not present in the Category dropdown`,
        );
      }
...
When('Click the "Delete" button on a {string} row', (plantName) => {
  pendingDeletePlantName = plantName;
});

When("Confirm the deletion on the popup", () => {
  const target = pendingDeletePlantName;
  cy.on("window:confirm", () => true);
  if (target) plantPage.clickDeleteForPlant(target);
});
...
Then(
  ...
  (expectedText) => {
    plantPage.assertOnlyRowsContainText(expectedText);
  },
);
...
Then("The plant list is refreshed", () => {
  plantPage.visitPlantPage();
  plantPage.assertPlantsTableVisible();
  cy.contains("table", createdPlantName, { timeout: 10000 }).should(
    "be.visible",
  );
});

Then('The "Low" stock badge is displayed for {string}', (plantName) => {
  plantPage.assertBadgeForPlant(plantName, "Low");
});

Then('The "Edit" icon is visible for {string}', (plantName) => {
  return ensurePlantExistsInList(plantName).then(() => {
    plantPage.assertActionVisibleForPlant(plantName, "Edit");
  });
});

Then('The "Delete" icon is visible for {string}', (plantName) => {
  return ensurePlantExistsInList(plantName).then(() => {
    plantPage.assertActionVisibleForPlant(plantName, "Delete");
  });
});
...
Then(
  ...
  (category, plantName) => {
    return plantPage.getColumnIndexByHeader("Category").then((colIndex) => {
      return cy
        .get("table tbody tr")
        .contains("td", plantName)
        .parent("tr")
        .should(($row) => {
          const text = plantPage.getCellText($row[0], colIndex);
          expect(plantPage.normalizeSpaces(text).toLowerCase()).to.include(
            plantPage.normalizeSpaces(category).toLowerCase(),
          );
        });
    });
  },
...
Then('The "Add a Plant" button is NOT visible', () => {
  plantPage.assertAddPlantButtonHidden();
});
...
Then(
  ...
  (category) => {
    return plantPage.assertOnlyRowsMatchCategory(category);
  },
);
